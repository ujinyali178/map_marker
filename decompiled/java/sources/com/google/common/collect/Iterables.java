...
public final class Iterables {
    ...
    public static <T> T get(Iterable<T> iterable, int i3) {
        Preconditions.checkNotNull(iterable);
        return iterable instanceof List ? (T) ((List) iterable).get(i3) : (T) Iterators.get(iterable.iterator(), i3);
    }

    @ParametricNullness
    public static <T> T get(Iterable<? extends T> iterable, int i3, @ParametricNullness T t3) {
        Preconditions.checkNotNull(iterable);
        Iterators.checkNonnegative(i3);
        if (iterable instanceof List) {
            List cast = Lists.cast(iterable);
            return i3 < cast.size() ? (T) cast.get(i3) : t3;
        }
    ...
    public static <T> Iterable<T> mergeSorted(final Iterable<? extends Iterable<? extends T>> iterable, final Comparator<? super T> comparator) {
        ...
        return new UnmodifiableIterable(new FluentIterable<T>() { // from class: com.google.common.collect.Iterables.9
            @Override // java.lang.Iterable
            public Iterator<T> iterator() {
                return Iterators.mergeSorted(Iterables.transform(iterable, new d()), comparator);
            }
        });
    }
