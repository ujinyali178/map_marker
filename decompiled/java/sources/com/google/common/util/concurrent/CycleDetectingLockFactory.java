...
public class CycleDetectingLockFactory {
    ...
    private static class ExampleStackTrace extends IllegalStateException {
        ...
        ExampleStackTrace(LockGraphNode lockGraphNode, LockGraphNode lockGraphNode2) {
            ...
            for (int i3 = 0; i3 < length; i3++) {
                if (WithExplicitOrdering.class.getName().equals(stackTrace[i3].getClassName())) {
                    setStackTrace(EMPTY_STACK_TRACE);
                    return;
                } else {
                    if (!EXCLUDED_CLASS_NAMES.contains(stackTrace[i3].getClassName())) {
    ...
    public static final class WithExplicitOrdering<E extends Enum<E>> extends CycleDetectingLockFactory {
        ...
        public ReentrantLock newReentrantLock(E e3, boolean z3) {
            ...
            LockGraphNode lockGraphNode = this.lockGraphNodes.get(e3);
    ...
    static <E extends Enum<E>> Map<E, LockGraphNode> createNodes(Class<E> cls) {
        ...
        for (E e3 : enumConstants) {
            LockGraphNode lockGraphNode = new LockGraphNode(getLockName(e3));
            newArrayListWithCapacity.add(lockGraphNode);
            newEnumMap.put((EnumMap) e3, (E) lockGraphNode);
        }
        for (int i4 = 1; i4 < length; i4++) {
            ((LockGraphNode) newArrayListWithCapacity.get(i4)).checkAcquiredLocks(Policies.THROW, newArrayListWithCapacity.subList(0, i4));
        }
        while (i3 < length - 1) {
            i3++;
            ((LockGraphNode) newArrayListWithCapacity.get(i3)).checkAcquiredLocks(Policies.DISABLED, newArrayListWithCapacity.subList(i3, length));
        }
    ...
    public ReentrantLock newReentrantLock(String str, boolean z3) {
        return this.policy == Policies.DISABLED ? new ReentrantLock(z3) : new CycleDetectingReentrantLock(new LockGraphNode(str), z3);
    }
    ...
    public ReentrantReadWriteLock newReentrantReadWriteLock(String str, boolean z3) {
        return this.policy == Policies.DISABLED ? new ReentrantReadWriteLock(z3) : new CycleDetectingReentrantReadWriteLock(new LockGraphNode(str), z3);
    }
}
