...
public final class MediaType {
    ...
    private static final class Tokenizer {
        ...
        char consumeCharacter(char c3) {
            Preconditions.checkState(hasMore());
            Preconditions.checkState(previewChar() == c3);
            this.position++;
            return c3;
        }
        ...
        String consumeToken(CharMatcher charMatcher) {
            ...
            Preconditions.checkState(this.position != i3);
        ...
        String consumeTokenIfPresent(CharMatcher charMatcher) {
            Preconditions.checkState(hasMore());
            int i3 = this.position;
            this.position = charMatcher.negate().indexIn(this.input, i3);
            return hasMore() ? this.input.substring(i3, this.position) : this.input.substring(i3);
        }

        boolean hasMore() {
            int i3 = this.position;
            return i3 >= 0 && i3 < this.input.length();
        }
    ...
    public static MediaType parse(String str) {
        ...
        try {
            ...
            while (tokenizer.hasMore()) {
                ...
                tokenizer.consumeCharacter(Tag.KEY_VALUE_SEPARATOR);
