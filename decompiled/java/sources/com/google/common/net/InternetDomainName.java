...
public final class InternetDomainName {
    ...
    private InternetDomainName ancestor(int i3) {
        ...
        return from(joiner.join(immutableList.subList(i3, immutableList.size())));
    }

    private int findSuffixOfType(Optional<PublicSuffixType> optional) {
        int size = this.parts.size();
        for (int i3 = 0; i3 < size; i3++) {
            String join = DOT_JOINER.join(this.parts.subList(i3, size));
    ...
    private static boolean validatePart(String str, boolean z3) {
        if (str.length() >= 1 && str.length() <= MAX_DOMAIN_PART_LENGTH) {
            ...
            if (!charMatcher.matches(str.charAt(0)) && !charMatcher.matches(str.charAt(str.length() - 1))) {
                return (z3 && DIGIT_MATCHER.matches(str.charAt(0))) ? false : true;
            }
        }
        return false;
    }
