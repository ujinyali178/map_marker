...
public final class PrfKeyTemplates {
    ...
    private static KeyTemplate createHmacTemplate(int i3, HashType hashType) {
        return KeyTemplate.newBuilder().setTypeUrl(new HmacPrfKeyManager().getKeyType()).setValue(HmacPrfKeyFormat.newBuilder().setParams(HmacPrfParams.newBuilder().setHash(hashType).build()).setKeySize(i3).build().toByteString()).setOutputPrefixType(OutputPrefixType.RAW).build();
    }
}
