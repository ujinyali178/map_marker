...
final class ByteBufferWriter {
    ...
    private static byte[] getOrCreateBuffer(int i3) {
    ...
    private static boolean needToReallocate(int i3, int i4) {
        return i4 < i3 && ((float) i4) < ((float) i3) * BUFFER_REALLOCATION_THRESHOLD;
    }
    ...
    private static boolean writeToChannel(ByteBuffer byteBuffer, OutputStream outputStream) {
        long j3 = CHANNEL_FIELD_OFFSET;
        if (j3 < CHANNEL_FIELD_OFFSET || !FILE_OUTPUT_STREAM_CLASS.isInstance(outputStream)) {
            return false;
        }
        WritableByteChannel writableByteChannel = null;
        try {
            writableByteChannel = (WritableByteChannel) UnsafeUtil.getObject(outputStream, j3);
