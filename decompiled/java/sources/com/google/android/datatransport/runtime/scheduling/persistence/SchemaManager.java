...
final class SchemaManager extends SQLiteOpenHelper {
    ...
    @Inject
    SchemaManager(Context context, @Named("SQLITE_DB_NAME") String str, @Named("SCHEMA_VERSION") int i3) {
    ...
    private void onCreate(SQLiteDatabase sQLiteDatabase, int i3) {
        ensureConfigured(sQLiteDatabase);
        upgrade(sQLiteDatabase, 0, i3);
    }

    private void upgrade(SQLiteDatabase sQLiteDatabase, int i3, int i4) {
        List<Migration> list = INCREMENTAL_MIGRATIONS;
        if (i4 <= list.size()) {
            while (i3 < i4) {
                INCREMENTAL_MIGRATIONS.get(i3).upgrade(sQLiteDatabase);
                i3++;
            }
            return;
        }
