...
public class SafeParcelReader {
    ...
    public static PendingIntent readPendingIntent(Parcel parcel, int i3) {
        int readSize = readSize(parcel, i3);
        int dataPosition = parcel.dataPosition();
        if (readSize == 0) {
            return null;
    ...
    private static void zzb(Parcel parcel, int i3, int i4) {
        int readSize = readSize(parcel, i3);
        if (readSize == i4) {
            return;
        }
        throw new ParseException("Expected size " + i4 + " got " + readSize + " (0x" + Integer.toHexString(readSize) + ")", parcel);
    }
}
