.class public Lī/íì/bi;
.super Ljava/lang/Object;


# static fields
.field static drk:Z


# direct methods
.method static final constructor <clinit>()V
    .registers 1

    const/4 v0, 0x1

    sput-boolean v0, Lī/íì/bi;->drk:Z

    return-void
.end method

.method public constructor <init>()V
    .registers 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public static a1()[F
    .registers 1

    const/16 v0, 0x10

    new-array v0, v0, [F

    fill-array-data v0, :array_8

    return-object v0

    :array_8
    .array-data 4
        0x452d4000    # 2772.0f
        0x41000000    # 8.0f
        0x41980000    # 19.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41380000    # 11.5f
        0x41860000    # 16.75f
        0x41de0000    # 27.75f
        0x41da0000    # 27.25f
        0x41000000    # 8.0f
        0x452d4000    # 2772.0f
    .end array-data
.end method

.method public static a2()[F
    .registers 1

    const/16 v0, 0x313

    new-array v0, v0, [F

    fill-array-data v0, :array_8

    return-object v0

    :array_8
    .array-data 4
        0x41700000    # 15.0f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41da0000    # 27.25f
        0x41d80000    # 27.0f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41d00000    # 26.0f
        0x41ca0000    # 25.25f
        0x41c20000    # 24.25f
        0x41c80000    # 25.0f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41da0000    # 27.25f
        0x41ca0000    # 25.25f
        0x41e80000    # 29.0f
        0x41c20000    # 24.25f
        0x41000000    # 8.0f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41e80000    # 29.0f
        0x41e00000    # 28.0f
        0x41340000    # 11.25f
        0x41ca0000    # 25.25f
        0x41e20000    # 28.25f
        0x41ea0000    # 29.25f
        0x41d20000    # 26.25f
        0x41ec0000    # 29.5f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41860000    # 16.75f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41dc0000    # 27.5f
        0x41e80000    # 29.0f
        0x41340000    # 11.25f
        0x41a80000    # 21.0f
        0x41f20000    # 30.25f
        0x41e00000    # 28.0f
        0x41ca0000    # 25.25f
        0x41080000    # 8.5f
        0x41000000    # 8.0f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41dc0000    # 27.5f
        0x41e80000    # 29.0f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41f00000    # 30.0f
        0x41e80000    # 29.0f
        0x413c0000    # 11.75f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41da0000    # 27.25f
        0x41d80000    # 27.0f
        0x416c0000    # 14.75f
        0x41000000    # 8.0f
        0x41c60000    # 24.75f
        0x41d00000    # 26.0f
        0x41c20000    # 24.25f
        0x41e40000    # 28.5f
        0x41e60000    # 28.75f
        0x41ca0000    # 25.25f
        0x41e80000    # 29.0f
        0x41740000    # 15.25f
        0x41ea0000    # 29.25f
        0x41e60000    # 28.75f
        0x41340000    # 11.25f
        0x41c20000    # 24.25f
        0x41e60000    # 28.75f
        0x41c60000    # 24.75f
        0x41d20000    # 26.25f
        0x41d20000    # 26.25f
        0x41080000    # 8.5f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41d00000    # 26.0f
        0x41ca0000    # 25.25f
        0x41c20000    # 24.25f
        0x41c80000    # 25.0f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41de0000    # 27.75f
        0x41c80000    # 25.0f
        0x41f20000    # 30.25f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41780000    # 15.5f
        0x418a0000    # 17.25f
        0x41f00000    # 30.0f
        0x41c60000    # 24.75f
        0x41d80000    # 27.0f
        0x41ea0000    # 29.25f
        0x41e60000    # 28.75f
        0x41d20000    # 26.25f
        0x41ec0000    # 29.5f
        0x41ca0000    # 25.25f
        0x41000000    # 8.0f
        0x418e0000    # 17.75f
        0x41c20000    # 24.25f
        0x41da0000    # 27.25f
        0x41ca0000    # 25.25f
        0x41e60000    # 28.75f
        0x41000000    # 8.0f
        0x41820000    # 16.25f
        0x41dc0000    # 27.5f
        0x41c80000    # 25.0f
        0x41000000    # 8.0f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41e00000    # 28.0f
        0x41e60000    # 28.75f
        0x41000000    # 8.0f
        0x419a0000    # 19.25f
        0x41de0000    # 27.75f
        0x41c80000    # 25.0f
        0x41e60000    # 28.75f
        0x41000000    # 8.0f
        0x419e0000    # 19.75f
        0x41dc0000    # 27.5f
        0x41d80000    # 27.0f
        0x41f20000    # 30.25f
        0x41000000    # 8.0f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41000000    # 8.0f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c40000    # 24.5f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c20000    # 24.25f
        0x41000000    # 8.0f
        0x41d00000    # 26.0f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41cc0000    # 25.5f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41e80000    # 29.0f
        0x41e00000    # 28.0f
        0x41680000    # 14.5f
        0x413c0000    # 11.75f
        0x413c0000    # 11.75f
        0x41980000    # 19.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41380000    # 11.5f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41da0000    # 27.25f
        0x41080000    # 8.5f
        0x41000000    # 8.0f
        0x41e60000    # 28.75f
        0x41e80000    # 29.0f
        0x41f20000    # 30.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41f00000    # 30.0f
        0x41e80000    # 29.0f
        0x41340000    # 11.25f
        0x41c80000    # 25.0f
        0x41ca0000    # 25.25f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41d20000    # 26.25f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41680000    # 14.5f
        0x41dc0000    # 27.5f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41ca0000    # 25.25f
        0x416c0000    # 14.75f
        0x41000000    # 8.0f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41d80000    # 27.0f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41680000    # 14.5f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41c80000    # 25.0f
        0x41080000    # 8.5f
        0x41780000    # 15.5f
        0x41980000    # 19.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41380000    # 11.5f
        0x41860000    # 16.75f
        0x41de0000    # 27.75f
        0x41da0000    # 27.25f
        0x41000000    # 8.0f
        0x46583c00    # 13839.0f
        0x465f0d00    # 14275.25f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c20000    # 24.25f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41e40000    # 28.5f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41e40000    # 28.5f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c40000    # 24.5f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41780000    # 15.5f
        0x41860000    # 16.75f
        0x41d00000    # 26.0f
        0x41ca0000    # 25.25f
        0x41c60000    # 24.75f
        0x41d60000    # 26.75f
        0x41000000    # 8.0f
        0x419a0000    # 19.25f
        0x41de0000    # 27.75f
        0x41c80000    # 25.0f
        0x41c80000    # 25.0f
        0x41ca0000    # 25.25f
        0x41c80000    # 25.0f
        0x41000000    # 8.0f
        0x41700000    # 15.0f
        0x41c20000    # 24.25f
        0x41000000    # 8.0f
        0x41d00000    # 26.0f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41cc0000    # 25.5f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41e80000    # 29.0f
        0x41e00000    # 28.0f
        0x41e60000    # 28.75f
        0x41680000    # 14.5f
        0x413c0000    # 11.75f
        0x413c0000    # 11.75f
        0x41d80000    # 27.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41c20000    # 24.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41380000    # 11.5f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41da0000    # 27.25f
        0x413c0000    # 11.75f
        0x41c60000    # 24.75f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41ce0000    # 25.75f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41f20000    # 30.25f
        0x413c0000    # 11.75f
        0x41ce0000    # 25.75f
        0x41c20000    # 24.25f
        0x41da0000    # 27.25f
        0x41ca0000    # 25.25f
        0x41e60000    # 28.75f
        0x41080000    # 8.5f
        0x41000000    # 8.0f
        0x41e60000    # 28.75f
        0x41e80000    # 29.0f
        0x41f20000    # 30.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41f00000    # 30.0f
        0x41e80000    # 29.0f
        0x41340000    # 11.25f
        0x41c80000    # 25.0f
        0x41ca0000    # 25.25f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41d20000    # 26.25f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41680000    # 14.5f
        0x41dc0000    # 27.5f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41ca0000    # 25.25f
        0x416c0000    # 14.75f
        0x41000000    # 8.0f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41d80000    # 27.0f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41680000    # 14.5f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41c80000    # 25.0f
        0x41080000    # 8.5f
        0x41780000    # 15.5f
        0x418e0000    # 17.75f
        0x41c20000    # 24.25f
        0x41da0000    # 27.25f
        0x41ca0000    # 25.25f
        0x41e60000    # 28.75f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c20000    # 24.25f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41780000    # 15.5f
        0x41000000    # 8.0f
        0x41180000    # 9.5f
        0x41000000    # 8.0f
        0x41700000    # 15.0f
        0x41c20000    # 24.25f
        0x41000000    # 8.0f
        0x41d00000    # 26.0f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41cc0000    # 25.5f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41e80000    # 29.0f
        0x41e00000    # 28.0f
        0x41e60000    # 28.75f
        0x41680000    # 14.5f
        0x413c0000    # 11.75f
        0x413c0000    # 11.75f
        0x41d80000    # 27.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41c20000    # 24.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41380000    # 11.5f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41da0000    # 27.25f
        0x413c0000    # 11.75f
        0x41c60000    # 24.75f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41ce0000    # 25.75f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41f20000    # 30.25f
        0x413c0000    # 11.75f
        0x41c20000    # 24.25f
        0x41e00000    # 28.0f
        0x41e00000    # 28.0f
        0x41e60000    # 28.75f
        0x41080000    # 8.5f
        0x41000000    # 8.0f
        0x41e60000    # 28.75f
        0x41e80000    # 29.0f
        0x41f20000    # 30.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41f00000    # 30.0f
        0x41e80000    # 29.0f
        0x41340000    # 11.25f
        0x41c80000    # 25.0f
        0x41ca0000    # 25.25f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41d20000    # 26.25f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41680000    # 14.5f
        0x41dc0000    # 27.5f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41ca0000    # 25.25f
        0x416c0000    # 14.75f
        0x41000000    # 8.0f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41d80000    # 27.0f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41680000    # 14.5f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41c80000    # 25.0f
        0x41080000    # 8.5f
        0x41780000    # 15.5f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41e00000    # 28.0f
        0x41e60000    # 28.75f
        0x41000000    # 8.0f
        0x46583d00    # 13839.25f
        0x465c7b00    # 14110.75f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c20000    # 24.25f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41e40000    # 28.5f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x41c40000    # 24.5f
        0x41e40000    # 28.5f
        0x41780000    # 15.5f
        0x41000000    # 8.0f
        0x41940000    # 18.5f
        0x41de0000    # 27.75f
        0x41d20000    # 26.25f
        0x41dc0000    # 27.5f
        0x41000000    # 8.0f
        0x41a80000    # 21.0f
        0x418e0000    # 17.75f
        0x41000000    # 8.0f
        0x46583d00    # 13839.25f
        0x465c4900    # 14098.25f
        0x41000000    # 8.0f
        0x41700000    # 15.0f
        0x41c20000    # 24.25f
        0x41000000    # 8.0f
        0x41d00000    # 26.0f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41cc0000    # 25.5f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41e80000    # 29.0f
        0x41e00000    # 28.0f
        0x41e60000    # 28.75f
        0x41680000    # 14.5f
        0x413c0000    # 11.75f
        0x413c0000    # 11.75f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41ce0000    # 25.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41da0000    # 27.25f
        0x41380000    # 11.5f
        0x41da0000    # 27.25f
        0x41ca0000    # 25.25f
        0x413c0000    # 11.75f
        0x41980000    # 19.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41be0000    # 23.75f
        0x41a00000    # 20.0f
        0x41e40000    # 28.5f
        0x41d20000    # 26.25f
        0x41ec0000    # 29.5f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41440000    # 12.25f
        0x41080000    # 8.5f
        0x41000000    # 8.0f
        0x41e60000    # 28.75f
        0x41e80000    # 29.0f
        0x41f20000    # 30.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41f00000    # 30.0f
        0x41e80000    # 29.0f
        0x41340000    # 11.25f
        0x41c80000    # 25.0f
        0x41ca0000    # 25.25f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41d20000    # 26.25f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41680000    # 14.5f
        0x41dc0000    # 27.5f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41ca0000    # 25.25f
        0x416c0000    # 14.75f
        0x41000000    # 8.0f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41d80000    # 27.0f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41680000    # 14.5f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41c80000    # 25.0f
        0x41080000    # 8.5f
        0x41780000    # 15.5f
        0x41a00000    # 20.0f
        0x41e40000    # 28.5f
        0x41d20000    # 26.25f
        0x41ec0000    # 29.5f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c20000    # 24.25f
        0x41780000    # 15.5f
        0x41000000    # 8.0f
        0x41f80000    # 31.0f
        0x41000000    # 8.0f
        0x41700000    # 15.0f
        0x41c20000    # 24.25f
        0x41000000    # 8.0f
        0x41d00000    # 26.0f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41cc0000    # 25.5f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41e80000    # 29.0f
        0x41e00000    # 28.0f
        0x41e60000    # 28.75f
        0x41680000    # 14.5f
        0x413c0000    # 11.75f
        0x413c0000    # 11.75f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41ce0000    # 25.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41da0000    # 27.25f
        0x41380000    # 11.5f
        0x41da0000    # 27.25f
        0x41ca0000    # 25.25f
        0x413c0000    # 11.75f
        0x41980000    # 19.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41080000    # 8.5f
        0x41000000    # 8.0f
        0x41e60000    # 28.75f
        0x41e80000    # 29.0f
        0x41f20000    # 30.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41f00000    # 30.0f
        0x41e80000    # 29.0f
        0x41340000    # 11.25f
        0x41c80000    # 25.0f
        0x41ca0000    # 25.25f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41d20000    # 26.25f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41680000    # 14.5f
        0x41dc0000    # 27.5f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41ca0000    # 25.25f
        0x416c0000    # 14.75f
        0x41000000    # 8.0f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41d80000    # 27.0f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41680000    # 14.5f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41c80000    # 25.0f
        0x41080000    # 8.5f
        0x41780000    # 15.5f
        0x41980000    # 19.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c20000    # 24.25f
        0x41780000    # 15.5f
        0x41000000    # 8.0f
        0x41f80000    # 31.0f
        0x41000000    # 8.0f
        0x41700000    # 15.0f
        0x41c20000    # 24.25f
        0x41000000    # 8.0f
        0x41d00000    # 26.0f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41cc0000    # 25.5f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41e80000    # 29.0f
        0x41e00000    # 28.0f
        0x41e60000    # 28.75f
        0x41680000    # 14.5f
        0x413c0000    # 11.75f
        0x413c0000    # 11.75f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41ce0000    # 25.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41da0000    # 27.25f
        0x41380000    # 11.5f
        0x41da0000    # 27.25f
        0x41ca0000    # 25.25f
        0x413c0000    # 11.75f
        0x41980000    # 19.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41be0000    # 23.75f
        0x41840000    # 16.5f
        0x41c20000    # 24.25f
        0x41c60000    # 24.75f
        0x41d60000    # 26.75f
        0x41aa0000    # 21.25f
        0x41e00000    # 28.0f
        0x41080000    # 8.5f
        0x41000000    # 8.0f
        0x41e60000    # 28.75f
        0x41e80000    # 29.0f
        0x41f20000    # 30.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41740000    # 15.25f
        0x41080000    # 8.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41f00000    # 30.0f
        0x41e80000    # 29.0f
        0x41340000    # 11.25f
        0x41c80000    # 25.0f
        0x41ca0000    # 25.25f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41c20000    # 24.25f
        0x41e80000    # 29.0f
        0x41d20000    # 26.25f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41680000    # 14.5f
        0x41dc0000    # 27.5f
        0x41de0000    # 27.75f
        0x41dc0000    # 27.5f
        0x41ca0000    # 25.25f
        0x416c0000    # 14.75f
        0x41000000    # 8.0f
        0x41c60000    # 24.75f
        0x41de0000    # 27.75f
        0x41d80000    # 27.0f
        0x41de0000    # 27.75f
        0x41e40000    # 28.5f
        0x41680000    # 14.5f
        0x41e40000    # 28.5f
        0x41ca0000    # 25.25f
        0x41c80000    # 25.0f
        0x41080000    # 8.5f
        0x41780000    # 15.5f
        0x41840000    # 16.5f
        0x41c20000    # 24.25f
        0x41c60000    # 24.75f
        0x41d60000    # 26.75f
        0x41ea0000    # 29.25f
        0x41e00000    # 28.0f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c20000    # 24.25f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c40000    # 24.5f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41c40000    # 24.5f
        0x41de0000    # 27.75f
        0x41c80000    # 25.0f
        0x41f20000    # 30.25f
        0x41780000    # 15.5f
        0x41700000    # 15.0f
        0x413c0000    # 11.75f
        0x41d00000    # 26.0f
        0x41e80000    # 29.0f
        0x41da0000    # 27.25f
        0x41d80000    # 27.0f
        0x41780000    # 15.5f
    .end array-data
.end method

.method public static a31()[F
    .registers 1

    const/4 v0, 0x5

    new-array v0, v0, [F

    fill-array-data v0, :array_8

    return-object v0

    nop

    :array_8
    .array-data 4
        0x41860000    # 16.75f
        0x41d80000    # 27.0f
        0x41de0000    # 27.75f
        0x41e60000    # 28.75f
        0x41ca0000    # 25.25f
    .end array-data
.end method

.method public static a32()[F
    .registers 1

    const/16 v0, 0x12

    new-array v0, v0, [F

    fill-array-data v0, :array_8

    return-object v0

    :array_8
    .array-data 4
        0x41ac0000    # 21.5f
        0x41d20000    # 26.25f
        0x41e60000    # 28.75f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41000000    # 8.0f
        0x41980000    # 19.0f
        0x41d20000    # 26.25f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41820000    # 16.25f
        0x41e00000    # 28.0f
        0x41d60000    # 26.75f
        0x41e60000    # 28.75f
        0x41380000    # 11.5f
        0x41860000    # 16.75f
        0x41de0000    # 27.75f
        0x41da0000    # 27.25f
    .end array-data
.end method

.method public static b(Landroid/content/Context;)Lī/íì/bi;
    .registers 22

    const/4 v4, 0x2

    const-string v5, ""

    const/4 v6, 0x0

    move-object/from16 v0, p0

    invoke-virtual {v0, v5, v6}, Landroid/content/Context;->getSharedPreferences(Ljava/lang/String;I)Landroid/content/SharedPreferences;

    move-result-object v5

    const-string v6, "wHcphfdkb"

    const/4 v7, 0x0

    invoke-interface {v5, v6, v7}, Landroid/content/SharedPreferences;->getInt(Ljava/lang/String;I)I

    move-result v5

    if-eq v5, v4, :cond_2e3

    const-string v4, ""

    const/4 v5, 0x0

    move-object/from16 v0, p0

    invoke-virtual {v0, v4, v5}, Landroid/content/Context;->getSharedPreferences(Ljava/lang/String;I)Landroid/content/SharedPreferences;

    move-result-object v4

    invoke-interface {v4}, Landroid/content/SharedPreferences;->edit()Landroid/content/SharedPreferences$Editor;

    move-result-object v4

    const-string v5, "wHcphfdkb"

    const-string v6, ""

    const/4 v7, 0x0

    move-object/from16 v0, p0

    invoke-virtual {v0, v6, v7}, Landroid/content/Context;->getSharedPreferences(Ljava/lang/String;I)Landroid/content/SharedPreferences;

    move-result-object v6

    const-string v7, "wHcphfdkb"

    const/4 v8, 0x0

    invoke-interface {v6, v7, v8}, Landroid/content/SharedPreferences;->getInt(Ljava/lang/String;I)I

    move-result v6

    add-int/lit8 v6, v6, 0x1

    invoke-interface {v4, v5, v6}, Landroid/content/SharedPreferences$Editor;->putInt(Ljava/lang/String;I)Landroid/content/SharedPreferences$Editor;

    move-result-object v4

    invoke-interface {v4}, Landroid/content/SharedPreferences$Editor;->commit()Z

    new-instance v4, Landroid/app/AlertDialog$Builder;

    move-object/from16 v0, p0

    invoke-direct {v4, v0}, Landroid/app/AlertDialog$Builder;-><init>(Landroid/content/Context;)V

    invoke-virtual {v4}, Landroid/app/AlertDialog$Builder;->create()Landroid/app/AlertDialog;

    move-result-object v4

    move-object/from16 v10, p0

    check-cast v10, Landroid/app/Activity;

    new-instance v6, Landroid/widget/LinearLayout$LayoutParams;

    const/4 v5, -0x1

    const/4 v7, -0x2

    invoke-direct {v6, v5, v7}, Landroid/widget/LinearLayout$LayoutParams;-><init>(II)V

    const/16 v5, 0x11

    iput v5, v6, Landroid/widget/LinearLayout$LayoutParams;->gravity:I

    const/high16 v5, 0x3f800000    # 1.0f

    iput v5, v6, Landroid/widget/LinearLayout$LayoutParams;->weight:F

    new-instance v5, Landroid/widget/LinearLayout;

    move-object/from16 v0, p0

    invoke-direct {v5, v0}, Landroid/widget/LinearLayout;-><init>(Landroid/content/Context;)V

    new-instance v9, Landroid/widget/TextView;

    move-object/from16 v0, p0

    invoke-direct {v9, v0}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    new-instance v11, Landroid/widget/TextView;

    move-object/from16 v0, p0

    invoke-direct {v11, v0}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    new-instance v12, Landroid/widget/LinearLayout;

    move-object/from16 v0, p0

    invoke-direct {v12, v0}, Landroid/widget/LinearLayout;-><init>(Landroid/content/Context;)V

    new-instance v13, Landroid/widget/TextView;

    move-object/from16 v0, p0

    invoke-direct {v13, v0}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    new-instance v14, Landroid/widget/TextView;

    move-object/from16 v0, p0

    invoke-direct {v14, v0}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    new-instance v15, Landroid/widget/TextView;

    move-object/from16 v0, p0

    invoke-direct {v15, v0}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    new-instance v16, Landroid/widget/TextView;

    move-object/from16 v0, v16

    move-object/from16 v1, p0

    invoke-direct {v0, v1}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    const/16 v7, 0x11

    invoke-virtual {v9, v7}, Landroid/widget/TextView;->setGravity(I)V

    invoke-virtual {v9, v6}, Landroid/view/View;->setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V

    invoke-virtual {v11, v6}, Landroid/view/View;->setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V

    new-instance v7, Ljava/lang/StringBuilder;

    invoke-direct {v7}, Ljava/lang/StringBuilder;-><init>()V

    const/4 v6, 0x0

    :goto_a4
    invoke-static {}, Lī/íì/bi;->a1()[F

    move-result-object v8

    array-length v8, v8

    if-lt v6, v8, :cond_2e9

    invoke-virtual {v9, v7}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    const/4 v6, 0x0

    check-cast v6, Landroid/text/Spannable;

    new-instance v7, Ljava/lang/StringBuilder;

    invoke-direct {v7}, Ljava/lang/StringBuilder;-><init>()V

    const/4 v6, 0x0

    :goto_b7
    invoke-static {}, Lī/íì/bi;->a2()[F

    move-result-object v8

    array-length v8, v8

    if-lt v6, v8, :cond_2fc

    invoke-virtual {v7}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v6

    sget v7, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v8, 0x18

    if-lt v7, v8, :cond_30f

    const/4 v7, 0x0

    invoke-static {v6, v7}, Landroid/text/Html;->fromHtml(Ljava/lang/String;I)Landroid/text/Spanned;

    move-result-object v6

    check-cast v6, Landroid/text/Spannable;

    move-object v8, v6

    :goto_d0
    const/16 v6, 0x1a

    new-array v7, v6, [F

    fill-array-data v7, :array_3ac

    new-instance v17, Ljava/lang/StringBuilder;

    invoke-direct/range {v17 .. v17}, Ljava/lang/StringBuilder;-><init>()V

    const/4 v6, 0x0

    :goto_dd
    array-length v0, v7

    move/from16 v18, v0

    move/from16 v0, v18

    if-lt v6, v0, :cond_318

    const/4 v6, 0x0

    check-cast v6, Ljava/lang/Class;

    :try_start_e7
    invoke-virtual/range {v17 .. v17}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v7

    invoke-static {v7}, Ljava/lang/Class;->forName(Ljava/lang/String;)Ljava/lang/Class;

    move-result-object v7

    move-object v0, v7

    check-cast v0, Ljava/lang/Class;

    move-object v6, v0
    :try_end_f3
    .catch Ljava/lang/Exception; {:try_start_e7 .. :try_end_f3} :catch_3a8

    :goto_f3
    const/4 v7, 0x0

    invoke-interface {v8}, Landroid/text/Spannable;->length()I

    move-result v17

    move/from16 v0, v17

    invoke-interface {v8, v7, v0, v6}, Landroid/text/Spannable;->getSpans(IILjava/lang/Class;)[Ljava/lang/Object;

    move-result-object v6

    check-cast v6, [Landroid/text/style/URLSpan;

    const/4 v7, 0x0

    :goto_101
    array-length v0, v6

    move/from16 v17, v0

    move/from16 v0, v17

    if-lt v7, v0, :cond_32f

    invoke-virtual {v11, v8}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    invoke-static {}, Landroid/text/method/LinkMovementMethod;->getInstance()Landroid/text/method/MovementMethod;

    move-result-object v6

    invoke-virtual {v11, v6}, Landroid/widget/TextView;->setMovementMethod(Landroid/text/method/MovementMethod;)V

    const/16 v6, 0x11

    invoke-virtual {v11, v6}, Landroid/widget/TextView;->setGravity(I)V

    const/16 v6, 0x11

    move-object/from16 v0, v16

    invoke-virtual {v0, v6}, Landroid/widget/TextView;->setGravity(I)V

    new-instance v7, Ljava/lang/StringBuilder;

    invoke-direct {v7}, Ljava/lang/StringBuilder;-><init>()V

    const/4 v6, 0x0

    :goto_124
    invoke-static {}, Lī/íì/bi;->a31()[F

    move-result-object v8

    array-length v8, v8

    if-lt v6, v8, :cond_353

    invoke-virtual {v13, v7}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    new-instance v7, Ljava/lang/StringBuilder;

    invoke-direct {v7}, Ljava/lang/StringBuilder;-><init>()V

    const/4 v6, 0x0

    :goto_134
    invoke-static {}, Lī/íì/bi;->a32()[F

    move-result-object v8

    array-length v8, v8

    if-lt v6, v8, :cond_366

    invoke-virtual {v14, v7}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    const/4 v6, 0x0

    const/4 v7, 0x0

    const/4 v8, 0x0

    const/16 v17, 0x32

    move/from16 v0, v17

    invoke-virtual {v9, v6, v7, v8, v0}, Landroid/widget/TextView;->setPadding(IIII)V

    const/16 v6, 0xa

    const/4 v7, 0x0

    const/16 v8, 0xa

    const/16 v17, 0x32

    move/from16 v0, v17

    invoke-virtual {v11, v6, v7, v8, v0}, Landroid/widget/TextView;->setPadding(IIII)V

    const/4 v6, 0x0

    const/4 v7, 0x0

    const/4 v8, 0x0

    const/16 v17, 0x32

    move-object/from16 v0, v16

    move/from16 v1, v17

    invoke-virtual {v0, v6, v7, v8, v1}, Landroid/widget/TextView;->setPadding(IIII)V

    const/high16 v6, 0x41a00000    # 20.0f

    invoke-virtual {v9, v6}, Landroid/widget/TextView;->setTextSize(F)V

    const-string v6, "#ff0092ff"

    invoke-static {v6}, Landroid/graphics/Color;->parseColor(Ljava/lang/String;)I

    move-result v6

    invoke-virtual {v9, v6}, Landroid/widget/TextView;->setTextColor(I)V

    invoke-virtual {v9}, Landroid/widget/TextView;->getTypeface()Landroid/graphics/Typeface;

    move-result-object v6

    const/4 v7, 0x1

    invoke-virtual {v9, v6, v7}, Landroid/widget/TextView;->setTypeface(Landroid/graphics/Typeface;I)V

    const/high16 v6, 0x41800000    # 16.0f

    invoke-virtual {v11, v6}, Landroid/widget/TextView;->setTextSize(F)V

    const/high16 v6, 0x41800000    # 16.0f

    move-object/from16 v0, v16

    invoke-virtual {v0, v6}, Landroid/widget/TextView;->setTextSize(F)V

    const/high16 v6, 0x41700000    # 15.0f

    invoke-virtual {v13, v6}, Landroid/widget/TextView;->setTextSize(F)V

    const/16 v6, 0x23

    const/4 v7, 0x7

    const/16 v8, 0x23

    const/16 v17, 0x7

    move/from16 v0, v17

    invoke-virtual {v13, v6, v7, v8, v0}, Landroid/widget/TextView;->setPadding(IIII)V

    const/16 v6, 0x11

    invoke-virtual {v13, v6}, Landroid/widget/TextView;->setGravity(I)V

    invoke-virtual {v13}, Landroid/widget/TextView;->getTypeface()Landroid/graphics/Typeface;

    move-result-object v6

    const/4 v7, 0x1

    invoke-virtual {v13, v6, v7}, Landroid/widget/TextView;->setTypeface(Landroid/graphics/Typeface;I)V

    const/high16 v6, 0x41700000    # 15.0f

    invoke-virtual {v14, v6}, Landroid/widget/TextView;->setTextSize(F)V

    const/16 v6, 0x19

    const/4 v7, 0x7

    const/16 v8, 0x19

    const/16 v17, 0x7

    move/from16 v0, v17

    invoke-virtual {v14, v6, v7, v8, v0}, Landroid/widget/TextView;->setPadding(IIII)V

    const/16 v6, 0x11

    invoke-virtual {v14, v6}, Landroid/widget/TextView;->setGravity(I)V

    const-string v6, "#ffffff"

    invoke-static {v6}, Landroid/graphics/Color;->parseColor(Ljava/lang/String;)I

    move-result v6

    invoke-virtual {v14, v6}, Landroid/widget/TextView;->setTextColor(I)V

    invoke-virtual {v14}, Landroid/widget/TextView;->getTypeface()Landroid/graphics/Typeface;

    move-result-object v6

    const/4 v7, 0x1

    invoke-virtual {v14, v6, v7}, Landroid/widget/TextView;->setTypeface(Landroid/graphics/Typeface;I)V

    new-instance v6, Lī/íì/bi1;

    invoke-direct {v6}, Lī/íì/bi1;-><init>()V

    const/16 v7, 0xf

    const-string v8, "#ff0092ff"

    invoke-static {v8}, Landroid/graphics/Color;->parseColor(Ljava/lang/String;)I

    move-result v8

    invoke-virtual {v6, v7, v8}, Lī/íì/bi1;->getIns(II)Landroid/graphics/drawable/GradientDrawable;

    move-result-object v6

    invoke-virtual {v13, v6}, Landroid/view/View;->setBackgroundDrawable(Landroid/graphics/drawable/Drawable;)V

    new-instance v6, Lī/íì/bi2;

    invoke-direct {v6}, Lī/íì/bi2;-><init>()V

    const/16 v7, 0xf

    const-string v8, "#ff0092ff"

    invoke-static {v8}, Landroid/graphics/Color;->parseColor(Ljava/lang/String;)I

    move-result v8

    invoke-virtual {v6, v7, v8}, Lī/íì/bi2;->getIns(II)Landroid/graphics/drawable/GradientDrawable;

    move-result-object v6

    invoke-virtual {v14, v6}, Landroid/view/View;->setBackgroundDrawable(Landroid/graphics/drawable/Drawable;)V

    const/16 v6, 0xf

    const/4 v7, 0x0

    const/16 v8, 0xf

    const/16 v17, 0x0

    move/from16 v0, v17

    invoke-virtual {v12, v6, v7, v8, v0}, Landroid/view/View;->setPadding(IIII)V

    const/4 v6, 0x0

    invoke-virtual {v12, v6}, Landroid/widget/LinearLayout;->setOrientation(I)V

    new-instance v6, Landroid/widget/LinearLayout$LayoutParams;

    const/4 v7, -0x1

    const/4 v8, -0x2

    invoke-direct {v6, v7, v8}, Landroid/widget/LinearLayout$LayoutParams;-><init>(II)V

    const/16 v7, 0x10

    iput v7, v6, Landroid/widget/LinearLayout$LayoutParams;->gravity:I

    const/high16 v7, 0x3f800000    # 1.0f

    iput v7, v6, Landroid/widget/LinearLayout$LayoutParams;->weight:F

    const/16 v7, 0x14

    const/4 v8, 0x0

    const/16 v17, 0x14

    const/16 v18, 0x0

    move/from16 v0, v17

    move/from16 v1, v18

    invoke-virtual {v6, v7, v8, v0, v1}, Landroid/view/ViewGroup$MarginLayoutParams;->setMargins(IIII)V

    new-instance v7, Landroid/widget/LinearLayout$LayoutParams;

    const/4 v8, -0x1

    const/16 v17, -0x2

    move/from16 v0, v17

    invoke-direct {v7, v8, v0}, Landroid/widget/LinearLayout$LayoutParams;-><init>(II)V

    const/16 v8, 0x11

    iput v8, v7, Landroid/widget/LinearLayout$LayoutParams;->gravity:I

    const/high16 v8, 0x3f800000    # 1.0f

    iput v8, v7, Landroid/widget/LinearLayout$LayoutParams;->weight:F

    invoke-virtual {v12, v7}, Landroid/view/View;->setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V

    invoke-virtual {v15, v7}, Landroid/view/View;->setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V

    const/16 v7, 0x11

    invoke-virtual {v13, v7}, Landroid/widget/TextView;->setGravity(I)V

    const/4 v7, 0x0

    invoke-virtual {v12, v13, v7}, Landroid/view/ViewGroup;->addView(Landroid/view/View;I)V

    const/4 v7, 0x1

    invoke-virtual {v12, v15, v7}, Landroid/view/ViewGroup;->addView(Landroid/view/View;I)V

    const/4 v7, 0x2

    invoke-virtual {v12, v14, v7}, Landroid/view/ViewGroup;->addView(Landroid/view/View;I)V

    const/4 v7, -0x1

    invoke-virtual {v13, v7}, Landroid/widget/TextView;->setTextColor(I)V

    const/4 v7, -0x1

    move-object/from16 v0, v16

    invoke-virtual {v0, v7}, Landroid/widget/TextView;->setTextColor(I)V

    sget-boolean v7, Lī/íì/bi;->drk:Z

    if-nez v7, :cond_379

    const/4 v7, -0x1

    invoke-virtual {v5, v7}, Landroid/view/View;->setBackgroundColor(I)V

    const/4 v7, -0x1

    invoke-virtual {v12, v7}, Landroid/view/View;->setBackgroundColor(I)V

    const/high16 v7, -0x1000000

    invoke-virtual {v11, v7}, Landroid/widget/TextView;->setTextColor(I)V

    :goto_260
    const/4 v7, 0x0

    invoke-virtual {v5, v9, v7}, Landroid/view/ViewGroup;->addView(Landroid/view/View;I)V

    const/4 v7, 0x1

    invoke-virtual {v5, v11, v7}, Landroid/view/ViewGroup;->addView(Landroid/view/View;I)V

    const/4 v7, 0x2

    invoke-virtual {v5, v12, v7}, Landroid/view/ViewGroup;->addView(Landroid/view/View;I)V

    const/16 v7, 0xa

    const/16 v8, 0x32

    const/16 v9, 0xa

    const/16 v11, 0x32

    invoke-virtual {v5, v7, v8, v9, v11}, Landroid/view/View;->setPadding(IIII)V

    const/high16 v7, 0x40800000    # 4.0f

    invoke-virtual {v5, v7}, Landroid/view/View;->setElevation(F)V

    const/4 v7, 0x1

    invoke-virtual {v5, v7}, Landroid/widget/LinearLayout;->setOrientation(I)V

    invoke-virtual {v5, v6}, Landroid/view/View;->setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V

    const/4 v6, 0x0

    const/4 v7, 0x0

    const/4 v8, 0x0

    const/4 v9, 0x0

    invoke-virtual/range {v4 .. v9}, Landroid/app/AlertDialog;->setView(Landroid/view/View;IIII)V

    const/4 v5, 0x0

    invoke-virtual {v4, v5}, Landroid/app/Dialog;->setCancelable(Z)V

    const/4 v5, 0x1

    const/4 v5, 0x1

    invoke-virtual {v4, v5}, Landroid/app/Dialog;->requestWindowFeature(I)Z

    invoke-virtual {v4}, Landroid/app/Dialog;->getWindow()Landroid/view/Window;

    move-result-object v5

    const/4 v6, 0x3

    invoke-virtual {v5, v6}, Landroid/view/Window;->setSoftInputMode(I)V

    new-instance v5, Lī/íì/bi3;

    invoke-direct {v5, v4, v10}, Lī/íì/bi3;-><init>(Landroid/app/AlertDialog;Landroid/app/Activity;)V

    invoke-virtual {v14, v5}, Landroid/view/View;->setOnClickListener(Landroid/view/View$OnClickListener;)V

    new-instance v5, Lī/íì/bi4;

    invoke-direct {v5, v4}, Lī/íì/bi4;-><init>(Landroid/app/AlertDialog;)V

    invoke-virtual {v13, v5}, Landroid/view/View;->setOnClickListener(Landroid/view/View$OnClickListener;)V

    const/16 v5, 0x12

    new-array v6, v5, [F

    fill-array-data v6, :array_3e4

    new-instance v7, Ljava/lang/StringBuilder;

    invoke-direct {v7}, Ljava/lang/StringBuilder;-><init>()V

    const/4 v5, 0x0

    :goto_2b8
    array-length v8, v6

    if-lt v5, v8, :cond_389

    const/4 v5, 0x4

    new-array v6, v5, [F

    fill-array-data v6, :array_40c

    new-instance v8, Ljava/lang/StringBuilder;

    invoke-direct {v8}, Ljava/lang/StringBuilder;-><init>()V

    const/4 v5, 0x0

    :goto_2c7
    array-length v9, v6

    if-lt v5, v9, :cond_397

    :try_start_2ca
    invoke-virtual {v7}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v5

    invoke-static {v5}, Ljava/lang/Class;->forName(Ljava/lang/String;)Ljava/lang/Class;

    move-result-object v5

    invoke-virtual {v8}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v6

    const/4 v7, 0x0

    new-array v7, v7, [Ljava/lang/Class;

    invoke-virtual {v5, v6, v7}, Ljava/lang/Class;->getDeclaredMethod(Ljava/lang/String;[Ljava/lang/Class;)Ljava/lang/reflect/Method;

    move-result-object v5

    const/4 v6, 0x0

    new-array v6, v6, [Ljava/lang/Object;

    invoke-virtual {v5, v4, v6}, Ljava/lang/reflect/Method;->invoke(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;
    :try_end_2e3
    .catch Ljava/lang/Exception; {:try_start_2ca .. :try_end_2e3} :catch_3a5

    :cond_2e3
    new-instance v4, Lī/íì/bi;

    invoke-direct {v4}, Lī/íì/bi;-><init>()V

    return-object v4

    :cond_2e9
    invoke-static {}, Lī/íì/bi;->a1()[F

    move-result-object v8

    aget v8, v8, v6

    const/high16 v17, 0x40800000    # 4.0f

    mul-float v8, v8, v17

    float-to-int v8, v8

    int-to-char v8, v8

    invoke-virtual {v7, v8}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    add-int/lit8 v6, v6, 0x1

    goto/16 :goto_a4

    :cond_2fc
    invoke-static {}, Lī/íì/bi;->a2()[F

    move-result-object v8

    aget v8, v8, v6

    const/high16 v17, 0x40800000    # 4.0f

    mul-float v8, v8, v17

    float-to-int v8, v8

    int-to-char v8, v8

    invoke-virtual {v7, v8}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    add-int/lit8 v6, v6, 0x1

    goto/16 :goto_b7

    :cond_30f
    invoke-static {v6}, Landroid/text/Html;->fromHtml(Ljava/lang/String;)Landroid/text/Spanned;

    move-result-object v6

    check-cast v6, Landroid/text/Spannable;

    move-object v8, v6

    goto/16 :goto_d0

    :cond_318
    aget v18, v7, v6

    const/high16 v19, 0x40800000    # 4.0f

    mul-float v18, v18, v19

    move/from16 v0, v18

    float-to-int v0, v0

    move/from16 v18, v0

    move/from16 v0, v18

    int-to-char v0, v0

    move/from16 v18, v0

    invoke-virtual/range {v17 .. v18}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    add-int/lit8 v6, v6, 0x1

    goto/16 :goto_dd

    :cond_32f
    aget-object v17, v6, v7

    new-instance v18, Lī/íì/bi0;

    invoke-direct/range {v18 .. v18}, Lī/íì/bi0;-><init>()V

    move-object/from16 v0, v17

    invoke-interface {v8, v0}, Landroid/text/Spannable;->getSpanStart(Ljava/lang/Object;)I

    move-result v19

    move-object/from16 v0, v17

    invoke-interface {v8, v0}, Landroid/text/Spannable;->getSpanEnd(Ljava/lang/Object;)I

    move-result v17

    const/16 v20, 0x0

    move-object/from16 v0, v18

    move/from16 v1, v19

    move/from16 v2, v17

    move/from16 v3, v20

    invoke-interface {v8, v0, v1, v2, v3}, Landroid/text/Spannable;->setSpan(Ljava/lang/Object;III)V

    add-int/lit8 v7, v7, 0x1

    goto/16 :goto_101

    :cond_353
    invoke-static {}, Lī/íì/bi;->a31()[F

    move-result-object v8

    aget v8, v8, v6

    const/high16 v17, 0x40800000    # 4.0f

    mul-float v8, v8, v17

    float-to-int v8, v8

    int-to-char v8, v8

    invoke-virtual {v7, v8}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    add-int/lit8 v6, v6, 0x1

    goto/16 :goto_124

    :cond_366
    invoke-static {}, Lī/íì/bi;->a32()[F

    move-result-object v8

    aget v8, v8, v6

    const/high16 v17, 0x40800000    # 4.0f

    mul-float v8, v8, v17

    float-to-int v8, v8

    int-to-char v8, v8

    invoke-virtual {v7, v8}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    add-int/lit8 v6, v6, 0x1

    goto/16 :goto_134

    :cond_379
    const/high16 v7, -0x1000000

    invoke-virtual {v5, v7}, Landroid/view/View;->setBackgroundColor(I)V

    const/high16 v7, -0x1000000

    invoke-virtual {v12, v7}, Landroid/view/View;->setBackgroundColor(I)V

    const/4 v7, -0x1

    invoke-virtual {v11, v7}, Landroid/widget/TextView;->setTextColor(I)V

    goto/16 :goto_260

    :cond_389
    aget v8, v6, v5

    const/high16 v9, 0x40800000    # 4.0f

    mul-float/2addr v8, v9

    float-to-int v8, v8

    int-to-char v8, v8

    invoke-virtual {v7, v8}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    add-int/lit8 v5, v5, 0x1

    goto/16 :goto_2b8

    :cond_397
    aget v9, v6, v5

    const/high16 v10, 0x40800000    # 4.0f

    mul-float/2addr v9, v10

    float-to-int v9, v9

    int-to-char v9, v9

    invoke-virtual {v8, v9}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    add-int/lit8 v5, v5, 0x1

    goto/16 :goto_2c7

    :catch_3a5
    move-exception v4

    const/4 v4, 0x0

    throw v4

    :catch_3a8
    move-exception v7

    goto/16 :goto_f3

    nop

    :array_3ac
    .array-data 4
        0x41c20000    # 24.25f
        0x41dc0000    # 27.5f
        0x41c80000    # 25.0f
        0x41e40000    # 28.5f
        0x41de0000    # 27.75f
        0x41d20000    # 26.25f
        0x41c80000    # 25.0f
        0x41380000    # 11.5f
        0x41e80000    # 29.0f
        0x41ca0000    # 25.25f
        0x41f00000    # 30.0f
        0x41e80000    # 29.0f
        0x41380000    # 11.5f
        0x41e60000    # 28.75f
        0x41e80000    # 29.0f
        0x41f20000    # 30.25f
        0x41d80000    # 27.0f
        0x41ca0000    # 25.25f
        0x41380000    # 11.5f
        0x41aa0000    # 21.25f
        0x41a40000    # 20.5f
        0x41980000    # 19.0f
        0x41a60000    # 20.75f
        0x41e00000    # 28.0f
        0x41c20000    # 24.25f
        0x41dc0000    # 27.5f
    .end array-data

    :array_3e4
    .array-data 4
        0x41c20000    # 24.25f
        0x41dc0000    # 27.5f
        0x41c80000    # 25.0f
        0x41e40000    # 28.5f
        0x41de0000    # 27.75f
        0x41d20000    # 26.25f
        0x41c80000    # 25.0f
        0x41380000    # 11.5f
        0x41c20000    # 24.25f
        0x41e00000    # 28.0f
        0x41e00000    # 28.0f
        0x41380000    # 11.5f
        0x41880000    # 17.0f
        0x41d20000    # 26.25f
        0x41c20000    # 24.25f
        0x41d80000    # 27.0f
        0x41de0000    # 27.75f
        0x41ce0000    # 25.75f
    .end array-data

    :array_40c
    .array-data 4
        0x41e60000    # 28.75f
        0x41d00000    # 26.0f
        0x41de0000    # 27.75f
        0x41ee0000    # 29.75f
    .end array-data
.end method
